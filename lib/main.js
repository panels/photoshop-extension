...
var sign = function (dir, params, password) {
  ...
  try {
    execSync('java -jar "' + ucf + '" -package -storetype PKCS12 -keystore ' + cert + ' -storepass ' + password + ' -tsa http://tsa.safecreative.org "' + output + '" -C "' + dir + '" .')
...
module.exports = function (params) {
  ...
  if (execSync('which composite')) {
    var icons = ['icon-dark', 'icon-light']
    var colors = ['white', 'yellow', 'tomato', 'steelblue', 'hotpink']
    var randomColor = function () {
      return colors[Math.floor(Math.random() * colors.length)]
    }
    icons.forEach(function (icon) {
      var originalPath = path.resolve(params.panel[icon])
